...
export default async function InvestorDashboardPage() {
  ...
  const [user, investorProfile] = await Promise.all([
    ...
    prisma.investorProfile.findUnique({
      ...
      select: { firmName: true, checkSizeRange: true, investmentStage: true, sectorsOfInterest: true },
    }),
  ]);
  ...
  return (
    <main className="mx-auto w-full max-w-5xl flex-1 px-6 py-10">
      <div className="overflow-hidden rounded-3xl border border-indigo-100 bg-white shadow-[0_24px_60px_rgba(15,23,42,0.08)]">
        <div className="bg-gradient-to-r from-fuchsia-600 to-indigo-600 p-8 text-white">
          <p className="text-xs uppercase tracking-[0.18em] text-fuchsia-100">Investor dashboard</p>
          <h1 className="mt-2 text-3xl font-semibold">Welcome, {user?.name ?? "Investor"}</h1>
          <p className="mt-2 text-fuchsia-100">
            Define investment thesis and discover opportunities aligned with your stage and sector.
          </p>
        </div>
        <div className="grid gap-4 p-8 md:grid-cols-2">
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
            <p className="text-xs uppercase tracking-wide text-slate-500">Firm</p>
            <p className="mt-2 text-lg font-semibold text-slate-900">
              {investorProfile?.firmName ?? "Complete onboarding"}
            </p>
            <p className="text-sm text-slate-600">
              Check size: {investorProfile?.checkSizeRange ?? "Not set"}
            </p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
            <p className="text-xs uppercase tracking-wide text-slate-500">Investment focus</p>
            <p className="mt-2 text-lg font-semibold text-slate-900">
              {investorProfile?.investmentStage ?? "Not set"}
            </p>
            <p className="text-sm text-slate-600">
              Sectors: {investorProfile?.sectorsOfInterest ?? "Not set"}
            </p>
