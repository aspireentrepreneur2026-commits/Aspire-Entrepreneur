...
export default async function DashboardPage() {
  ...
  const [feedPosts, publicFeed] = await Promise.all([
  ...
  return (
    <main className="mx-auto w-full max-w-7xl flex-1 px-6 py-10">
      <section className="mb-10 rounded-3xl border border-indigo-100 bg-white p-6 shadow-[0_16px_40px_rgba(15,23,42,0.08)]">
        ...
        <p className="mt-2 text-sm text-slate-600">
          Post updates with text, links, images, and videos. Everyone signed in can see company names and comment on
          posts.
        </p>
      ...
      <section className="mb-12 space-y-6">
        ...
        {feedViews.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-6 py-12 text-center text-sm text-slate-600">
            No posts yet — be the first to share an idea, milestone, or link with the community.
          </div>
        ) : (
          feedViews.map((post) => (
            <FeedPostCard
      ...
      <section>
        <h2 className="mb-4 text-lg font-semibold text-slate-900">Discover members</h2>
        <p className="mb-6 text-sm text-slate-600">
          Public-safe profiles of people and teams who completed onboarding.
        </p>
